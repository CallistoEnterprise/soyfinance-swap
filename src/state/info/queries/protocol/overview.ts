...
interface OverviewResponse {
  soySwapFactories: PancakeFactory[]
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatPancakeFactoryResponse(data?.soySwapFactories?.[0])
      const overviewData24 = formatPancakeFactoryResponse(data24?.soySwapFactories?.[0])
      const overviewData48 = formatPancakeFactoryResponse(data48?.soySwapFactories?.[0])
