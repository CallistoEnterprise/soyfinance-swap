...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    user: {
      ...
      totalCLO: numberOrNull(betResponse.user.totalCLO),
    },
  } as Bet
...
export const transformTotalWonResponse = (
  ...
  const totalCLO = marketResponse.totalCLO ? parseFloat(marketResponse.totalCLO) : 0
  const totalCLOTreasury = marketResponse.totalCLOTreasury ? parseFloat(marketResponse.totalCLOTreasury) : 0

  return Math.max(totalCLO - (totalCLOTreasury + houseRounds), 0)
}
